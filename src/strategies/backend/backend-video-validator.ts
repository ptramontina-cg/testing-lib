import {
  ...
  ALLOWED_HOSTED_VIDEO_FORMATS,
  ALLOWED_HOSTED_VIDEO_MIN_BITRATES_BY_RESOLUTION,
  ALLOWED_HOSTED_VIDEO_SIZES_PX,
...
export class BackendVideoValidator {
  ...
  validateFormat(formats: string[]): string | boolean {
    for (const format of formats) {
      if (ALLOWED_HOSTED_VIDEO_FORMATS.includes(normalizeFileType(format))) {
        return false;
      }
    }
  ...
  validateResolution(height: number): string | boolean {
    if (ALLOWED_HOSTED_VIDEO_SIZES_PX.includes(height ?? 0)) {
      return false;
    }
  ...
  validateBitRate(bitRate: number, height: number) {
    ...
    const allowedBitRate =
      ALLOWED_HOSTED_VIDEO_MIN_BITRATES_BY_RESOLUTION[
        height as keyof typeof ALLOWED_HOSTED_VIDEO_MIN_BITRATES_BY_RESOLUTION
      ];
